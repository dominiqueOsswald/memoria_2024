...
for (return_type in return_types) {
  for (year in start_year:end_year) {
    ...
    for (actual_region in start_region:end_region) {

      write_file_path <- sprintf("../../results/dea_te_region/%d_%s.csv", year, return_type)

      data <- read.csv(read_file_path, header = TRUE)
      data_by_region <- data %>%
                        filter(region == actual_region)

      if (nrow(data_by_region) <= 1) {
        # Skip the loop iteration if the number of rows is 1
        next
      }

      # Filter rows
      normalized_columns <- c(1, 10, 11, 12, 13, 14, 15)
      filtered_data <- data_by_region[, normalized_columns]
      ...
      hospitals_data$region <- actual_region
      ...
      year_data[[as.character(actual_region)]] <- output_data_normal
    }
    combined_dataframe <- bind_rows(year_data)
    write.csv(combined_dataframe, write_file_path, row.names = FALSE, quote = FALSE)
  }
}
