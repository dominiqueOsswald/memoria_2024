...
for (return_type in return_types) {
  for (year in start_year:end_year) {
    ...
    # Filter rows
    normalized_columns <- c(1, 10, 11, 12, 13, 14, 15, 16)
    filtered_data <- data[, normalized_columns]
    filtered_data[is.na(filtered_data)] <- 0.00
    filtered_data$hospital_discharge_normalized <- filtered_data$GRD * filtered_data$hospital_discharge_normalized
    
    # DEA input
    dea_data <- make_deadata(filtered_data, ni= 2, no= 5, dmus = 1,
    ...
    output_data_grd <- output_data[, c("hospital_id",
    ...
    # Save to file
    output_data_grd$hospital_id <- gsub("DMU", "", output_data_grd$hospital_id)
    write.csv(output_data_grd, write_file_grd_path, row.names = FALSE, quote = FALSE) 
  }
}
