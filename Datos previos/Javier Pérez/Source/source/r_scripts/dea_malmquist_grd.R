...
for (orientation_type in orientation_types) {
  for (return_type in return_types) {
    ...
    write_file_path <- sprintf("../../results/dea_malmquist_grd/malmquist_%s_%s.csv",
                               orientation_type,
                               return_type)
    
    hospital_data$hospital_discharge_normalized <- hospital_data$GRD * hospital_data$hospital_discharge_normalized
