...
for (region in regions) {
  ...
  for (year in start_year:end_year) {
    read_file_path <- sprintf("../../results/dea_te_region/%d_%d.csv", year, region)
...
for(region in names(df_input_list)) {
  write_input_file_path <- sprintf("../../results/jaccard_index_region/jaccard_te_input_%s.csv", region)
...
for(region in names(df_output_list)) {
  write_output_file_path <- sprintf("../../results/jaccard_index_region/jaccard_te_output_%s.csv", region)
  # Calculate Jaccard Index for each pair of data frames
...
for(region in names(df_directional_list)) {
  write_dir_file_path <- sprintf("../../results/jaccard_index_region/jaccard_te_dir_%s.csv", region)
  # Calculate Jaccard Index for each pair of data frames
